...
function Info(params) {
  ...
  useEffect(() => {
    API.get(`/api/client/${data.id_client}`)
      .then(res => {
        setClient(res.data);
      })
  ...
  return (
    <React.Fragment>
      ...
      <TableCell className="tableCell">{data.message}</TableCell>
    </React.Fragment>
  );
          ...
          {data.map((r) => (
            <TableRow key={r._id}>
              ...
              <TableCell className="tableCell">
                <button className="btn btn-danger rounded-circle" onClick={() => deleteReclamation(r._id)}>
