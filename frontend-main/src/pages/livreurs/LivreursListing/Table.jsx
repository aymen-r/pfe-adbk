        ...
        <TableBody>
          {data.map((r) => (
            <TableRow key={r._id}>
              ...
              <TableCell className="tableCell">{r.nom + " " + r.prenom}</TableCell>
              ...
              <TableCell className="tableCell">{r.evaluation_livreur}</TableCell>
              {/* edit and delete with rounded buttons */}
              <TableCell className="tableCell">
                <button className="btn btn-primary mx-2 rounded-circle" onClick={() => window.location.href = `/livreurs/view/${r._id}`}>
                  <i class="bi bi-eye-fill"></i>
                </button>
                <button className="btn btn-primary mx-2 rounded-circle" onClick={() => window.location.href = `/livreurs/edit/${r._id}`}>
                  <i class="bi bi-pencil"></i>
                </button>
                <button className="btn btn-danger rounded-circle" onClick={() => deleteLivreur(r._id)}>
